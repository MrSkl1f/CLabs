...
void input_children(struct child **children, int *err, int *count)
{
    ...
    *children = malloc(step * sizeof(struct child));
    ...
    {
        input_and_check_surname((&(*children)[*count]), check, err, &check_nobody);
        if (*err == OK && check_nobody)
        {    
            if (!input_and_check_sex((&(*children)[*count]), err) && !input_and_check_birthdate((&(*children)[*count]), err)
                && !input_and_check_marks((&(*children)[*count]), err))
            {
...
{
    if (scanf("%d", &(cur_child->cur_mark.count_of_marks)) == 1 && 
        cur_child->cur_mark.count_of_marks > 0)
        *err = OK;
    else
        *err = ERROR;
    return *err;
...
{
    if (scanf("%s", cur_child->birthday) == 1 && 
        check_line_of_birthday(cur_child->birthday) && convert_birthday_into_int(cur_child) && 
        check_converted_birthday(cur_child->cur_child))
        *err = OK;
    else
        *err = ERROR;
    return *err;
...
{
    if (scanf("%s", cur_child->sex) == 1 && create_new_word_for_sex(cur_child->sex) && 
        check_end_of_line(cur_child->sex))
        *err = OK;
    else
        *err = ERROR;
    return *err;
...
void input_and_check_surname(struct child *cur_child, char *check, int *err, int *check_nobody)
{
    if (scanf("%s", check) == 1)
    {
        if (create_new_low_word(check, "nobody"))
        {
            ...
            {
                malloc_field(cur_child);
                strcpy(cur_child->first_name, check);
            }  
            else
                *err = ERROR;
        }
